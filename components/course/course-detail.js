...
const CourseDetail = ({ course }) => {
  ...
  const outline = course.course_outline
    .split('\n')
    .map((line, index) => <p key={index}>{line}</p>)
  const sup = course.course_sup
    .split('\n')
    .map((line, index) => <p key={index}>{line}</p>)
  return (
    <div className="fontDarkBrown">
      ...
      <div className={`mt-5 mb-5 ${styles['course-content']}`}>
        <h6 className={`h6Bold mb-3 ${styles['title']}`}>
          ◼ <span>&nbsp;&nbsp;</span>課程介紹
        </h6>
        <div className={`mb-5 web-16px-md ${styles['detail']}`}>
        ...
        <div className={`web-16px-md ${styles['shop-area']}`}>
          <div className={`web-16px-md ${styles['name']}`}>
            {course.shop_name}
          </div>
          <div className={`web-16px-md ${styles['address']}`}>
            {course.shop_address}
          </div>
          <div className={`web-16px-md ${styles['phone']}`}>
            {course.shop_phone}
          </div>
        </div>
      </div>
